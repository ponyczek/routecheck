...
import type { VehiclesFiltersState } from './types';
import { defaultFilters } from './types';

/**
 * Hook do zarządzania filtrami pojazdów z synchronizacją URL
 ...
 */
export function useVehiclesFilters() {
  // Parse initial filters from URL
  const [filters, setFilters] = useState<VehiclesFiltersState>(() => {
    ...
    return {
      ...
      sortBy: (searchParams.get('sortBy') as 'registrationNumber' | 'createdAt') || defaultFilters.sortBy,
   ...
   */
  const updateFilters = useCallback((updates: Partial<VehiclesFiltersState>) => {
    setFilters((prev) => {
