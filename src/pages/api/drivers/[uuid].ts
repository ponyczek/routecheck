...
export const GET: APIRoute = async ({ locals, params }) => {
  try {
    ...
    if (error || !driver) {
      return jsonResponse({ code: "NOT_FOUND", message: "Kierowca nie został znaleziony" }, 404);
    }
...
export const PATCH: APIRoute = async ({ locals, params, request }) => {
  try {
    ...
    if (existingDriver.deleted_at !== null) {
      return jsonResponse({ code: "FORBIDDEN", message: "Nie można edytować usuniętego kierowcy" }, 403);
    }
    ...
    if (updateError) {
      ...
      return jsonResponse({ code: "INTERNAL_ERROR", message: "Failed to update driver" }, 500);
    }
...
export const DELETE: APIRoute = async ({ locals, params }) => {
  try {
    ...
    if (deleteError) {
      ...
      return jsonResponse({ code: "INTERNAL_ERROR", message: "Failed to delete driver" }, 500);
    }
