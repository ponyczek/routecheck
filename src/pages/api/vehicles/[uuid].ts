...
export const GET: APIRoute = async ({ locals, params }) => {
  try {
    ...
    if (!uuid) {
      return jsonResponse({ code: "VALIDATION_ERROR", message: "Missing uuid parameter" }, 400);
    }
    ...
    if (error || !vehicle) {
      return jsonResponse({ code: "NOT_FOUND", message: "Pojazd nie został znaleziony" }, 404);
    }
...
export const PATCH: APIRoute = async ({ locals, params, request }) => {
  try {
    ...
    if (existingVehicle.deleted_at !== null) {
      return jsonResponse({ code: "FORBIDDEN", message: "Nie można edytować usuniętego pojazdu" }, 403);
    }
    ...
    if (
      ...
      body.registrationNumber.toUpperCase() !== existingVehicle.registration_number.toUpperCase()
    ) {
    ...
    if (Object.keys(updates).length === 0) {
      return jsonResponse({ code: "VALIDATION_ERROR", message: "No fields to update" }, 400);
    }
    ...
    if (updateError) {
      ...
      return jsonResponse({ code: "INTERNAL_ERROR", message: "Failed to update vehicle" }, 500);
    }
...
export const DELETE: APIRoute = async ({ locals, params }) => {
  try {
    ...
    if (deleteError) {
      ...
      return jsonResponse({ code: "INTERNAL_ERROR", message: "Failed to delete vehicle" }, 500);
    }
